...
def place_order(prediction, confidence):
    ...
    if symbol_info is None:
        logging.error(f"[{strategy_name}] Symbol {symbol} hittades inte.")
        mt5.shutdown()
        return

    if not symbol_info.visible:
        logging.info(f"[{strategy_name}] Aktiverar symbol {symbol}.")
        mt5.symbol_select(symbol, True)

    # Kolla om det redan finns en öppen position
    positions = mt5.positions_get(symbol=symbol)
    if positions and len(positions) > 0:
        logging.info(f"[{strategy_name}] Redan {len(positions)} öppen position – ingen ny order skickad.")
    ...
    if result.retcode == mt5.TRADE_RETCODE_DONE:
        logging.info(f"[{strategy_name}] Order genomförd! Inträde: {price:.2f} SL: {sl:.2f} TP: {tp:.2f}")
