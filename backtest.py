...
def _execute_trade(test_data, i, entry_price, signal):
    """Execute a single long (signal==1) or short (signal==-1) trade; return return %."""
...
def run_backtest(strategy, data_frame):
    """Backtest: execute trades for buy (1) and sell (-1) signals."""
    trades = []
...
def run_backtest_with_probabilities(rule_strategy, lr_strategy, data_frame):
    """Backtest rule entries only; attach LR P(success | direction) for interval analysis."""
    trades = []
...
def group_trades_by_interval(trades):
    ...
    return {label: ts if len(ts) > 0 else None
